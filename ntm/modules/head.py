...
class NTMHead(nn.Module):
    ...
    def forward(self, controller_state, prev_weights, memory, data=None):
        ...
        self.comgra.register_tensor(f"{self.head_name}_{self.comgra.FAKE_ITERATION}_b", b_pre, recording_type='neurons')
        self.comgra.register_tensor(f"{self.head_name}_{self.comgra.FAKE_ITERATION}_g", g_pre, recording_type='neurons')
        self.comgra.register_tensor(f"{self.head_name}_{self.comgra.FAKE_ITERATION}_s", s_pre, recording_type='neurons')
        b = F.softplus(b_pre)
        g = F.sigmoid(g_pre)
        s = F.softmax(s_pre)
